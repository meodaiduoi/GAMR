class Individual:
    ...
    def generateMemset(self, k, pair_list,exist_servers, map_server_path, function, adj_matrix, delay_predict_matrix, band_width_predict_matrix, loss_predict_matrix):
        delay_mean = 0
        loss_mean = 0
        bandwidth_mean = 0
        for pair in pair_list:
            src_node = pair[0]
            dst_node = pair[1]
            if dst_node in exist_servers:
                new_path = function.update_Path(src_node, dst_node, map_server_path[dst_node], adj_matrix)
                self.chromosome.append(new_path)
                predict_delay = function.cal_delay(new_path[2], delay_predict_matrix)
                predict_bandwidth = function.cal_bandwidth(new_path[2], band_width_predict_matrix)
                predict_lossPacket = function.cal_packetLoss(new_path[2], loss_predict_matrix)
