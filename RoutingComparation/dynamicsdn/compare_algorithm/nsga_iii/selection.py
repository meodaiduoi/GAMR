...
# def find_ideal_point(indi_list):
#     'Finds the ideal point from a set individuals.'
#     current_ideal = [np.infty] * len(indi_list[0].objectives)
...
def find_ideal_point(indi_list):
    current_ideal = []
    for i in range(len(indi_list[0].objectives)):
        objective_temp = np.inf
        for ind in indi_list:
            objective_temp = min(objective_temp, ind.objectives[i])
        current_ideal.append(objective_temp)
    return current_ideal
...
def find_extreme_points(indi_list):
    '''Finds the individuals with extreme values for each objective function.'''
    if not indi_list:
        return []
    
    num_objectives = len(indi_list[0].objectives)
    
    extreme_individuals = []
    for o in range(num_objectives):
        # Sort individuals based on the o-th objective in descending order
        sorted_individuals = sorted(indi_list, key=lambda ind: ind.objectives[o], reverse=True)
        # Select the individual with the maximum value for the o-th objective
        extreme_individuals.append(sorted_individuals[0])
    
    return extreme_individuals
...
def construct_hyperplane(individuals, extreme_points):
    ...
    def has_duplicate_individuals(individuals):
        '''Checks if there are duplicate individuals.'''
        for i in range(len(individuals)):
            for j in range(i + 1, len(individuals)):
    ...
    if has_duplicate_individuals(extreme_points):
        # If extreme points have duplicates, set intercepts to extreme values
        intercepts = [extreme_points[m].objectives[m] for m in range(num_objs)]
    else:
        # If no duplicates, solve for the intercepts using linear algebra
        b = np.ones(num_objs)
...
def normalize_objectives(individuals, intercepts, ideal_point):
    ...
    for ind in individuals:
        ind.normalized_objectives = list([normalize_objective(ind, m,
...
def associate(individuals, reference_points):
    ...
    pareto_fronts = fast_nondominated_sort(individuals)
    num_objs = len(individuals[0].objectives)

    for ind in individuals:
        rp_dists = [(rp, perpendicular_distance(ind.normalized_objectives, rp))
